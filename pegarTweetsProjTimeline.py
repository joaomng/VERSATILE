...
def pegarTweetsUsuarioCovid19(clientOauth, user, sinceId = 0, maxId = 0, maxTweets=200):
    ...
    #Version 1.1
    URL = "https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name="+user+"&count="+str(maxTweets)+"&tweet_mode=extended"
    
    #new Twitter API version: 2
    #URL = "https://api.twitter.com/2/statuses/user_timeline.json?screen_name="+user+"&count="+str(maxTweets)
    ...
    response,data = clientOauth.request(URL,"GET")

    return response,data

'''
Este metodo e uma adaptacao da funcao pegarTweetsDeUmUsuario. A diferenca e que o que se deseja e pegar 
uma quantidade (max de 200) mensagens de uma timeline dado um intervalo. Esses dados sao sempre os mais
recentes na timeline ainda que sejam repetidos a cada coleta. Por exemplo, se eu pego 5 tweets mais 
recentes e rodo novamente em 1 minuto e apenas uma nova mensagem chegou, pegarei os 5 mais recentes (o novo
...
if __name__ == "__main__":
    ...
    arquivo = input("Forneca o nome do banco onde serao salvos os dados da coleta:")
    ...
    try:
        intervaloColeta = int(input("Qual o intervalo de coleta (em segundos)? (default 1500s)")) #3600
        qtdeTweetsPorRequisicao = int(input("Qual a quantidade de tweets deseja por requisicao (min = 1 e max = 200)?")) #30
    except:
        intervaloColeta = 3600
        qtdeTweetsPorRequisicao = 30

    resp = input("Deseja coletar da timeline do usuario dono das credenciais fornecidas? (S-sim, n-nao)")
    ...
    if (resp != 'n'):
        ...
        userName = input("Qual nome do usuario que voce deseja coletar dados da timeline (sem '@') ?")
